...
fixed_rows = [ 
                    ...
                    "Leasehold Land & Building", "Retention Advances", 
        ...
        "Net Profit/Loss frosm discontinued operations (+/-)",
        "Profit for the year",
...
def fix_json_parent_name(data, filename):
    ...
    update_notes_data_dict(data, filename)
    return data
        ...
        if filename.endswith(".json"):
            ...
            try:
                with open(file_path, "r") as json_file:
                    ...
                    update_notes_data_dict(modified_data, filename)  
...
def update_notes_data_dict(data, filename):
    """
    Updates the notes_data_dict with the filename for each separate dictionary.
    ...
    for parent_name, sub_data in data.items():
        notes_data_dict = {}
        if parent_name not in notes_data_dict:
            notes_data_dict[parent_name] = {}
        notes_data_dict[parent_name].update(sub_data)
        
        for year in sub_data.get("page number", {}):
            if year not in notes_data_dict[parent_name]:
                notes_data_dict[parent_name][year] = {}
            notes_data_dict[parent_name][year][filename] = filename
        ...
        if key in existing_data:
            if isinstance(value, dict):  
                for sub_key, sub_value in value.items():
                    ...
                    elif sub_key in existing_data[key]:
...
fixed_rows = {
    "Assets": ["CURRENT ASSETS","Cash in the bank & hand", "Cash and cash equivalents", "Trade debtors", "Tax receivables", 
               "Trade Debtors - Related party", "Provision for bad debts", 
               "Stocks / Inventory", "Due from connected companies", 
               "Due from Directors", "Deposits & Prepayments", 
               "Leasehold Land & Building", "Retention Advances", 
    ...
    "Liabilities": ["CURRENT LIABILITIES","Prepayments and other receivables","Due to banks- Overdrafts", "Due to banks-Short term borrowings", 
                    "Current Maturity on term loans", "Obligations under finance lease <1 Yr.", 
                    "Other Financial Liabilities", "Trade Creditors", "Trade Creditors - Related Party", 
                    "Due to connected companies", "Due to Directors", "Dividend / Tax payable", 
                    "Retentions", "Advances From Clients", "Mobilization Advances on Contracts", 
    ...
    "FIXED & TERM ASSETS": [
        ...
        "Investments (Incl.Debentures,Shares etc.)",
        "Investment in connected companies",
        "Lease/Hire Purchase receivables >1Yr",
        "Other receivables > 1Yr",
    ...
    "LONG TERM LIABILITIES": [
        "LONG TERM LIABILITIES",
        "Bank borrowings",
        "Obligations under finance lease > 1 Yr.",
        "Related party borrowings",
        "Deposits maturing > 1Yr.",
        "Debentures",
        "Deferred tax",
...
excel_file = "bsr/excel_files/balance_sheet.xlsx"

with pd.ExcelWriter(excel_file, engine='openpyxl') as writer:
    if combined_data_bs:  
        df.to_excel(writer, sheet_name=sheet_name, index_label="BALANCE SHEET", startrow=2)
...
topic_cell1 = ws.cell(row=1, column=1)
...
ws.merge_cells(topic_range)

topic_cell2 = ws2.cell(row=1, column=1)
topic_cell3 = ws2.cell(row=2, column=3)
...
for row in ws.iter_rows(min_row=3, max_row=max_row, min_col=1, max_col=max_column):
    for cell in row:
...
for row in ws.iter_rows(min_row=3, max_row=500, min_col=1, max_col=max_column):
    for cell in row:
        for section, rows in fixed_rows.items():
...
for row in ws2.iter_rows(min_row=3, max_row=500 , min_col=1, max_col=max_column):
    for cell in row:
        for section, rows in fixed_rows.items():
            if cell.value in rows:
                cell.alignment = Alignment(horizontal='left')
...
for column_cells in ws.columns:
    ...
    ws.column_dimensions[column_cells[1].column_letter].width = length + 2  

for column_cells in ws2.columns:
    length = max(len(str(cell.value)) for cell in column_cells)
    ws2.column_dimensions[column_cells[1].column_letter].width = length + 2  
